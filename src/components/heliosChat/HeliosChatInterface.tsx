...
import { Send, X, Bot, User2, Check, Info, Sparkles, RotateCcw } from "lucide-react";
import { useHeliosChat, quickCreditSuggestions } from "@/contexts/HeliosChatContext";


export interface HeliosChatProps{
    onClose?:()=>void;
    isMobile?:boolean;
}
...
const containerVariants = {
  ...
  const HeliosChatInterface = ({ onClose, isMobile }: HeliosChatProps) => {
    const [isInfoModalOpen, setIsInfoModalOpen] = useState(false);
    const {messages,isTyping,clearChatHistory,startNewChat, sendMessage} = useHeliosChat()
        ...
        <div className="bg-background border border-border rounded-xl shadow-xl overflow-hidden h-[750px] flex flex-col">
          ...
          <div className="flex items-center justify-between p-4 border-b border-border bg-gradient-to-r from-primary/5 to-accent/5">
            <div className="flex items-center">
              ...
              <div>
                <h3 className="font-medium text-foreground">Asistente Crediticio</h3>
              ...
              {messages.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground text-center max-w-xs mb-6">
                    Soy tu asistente crediticio virtual. ¿En qué puedo ayudarte hoy?
                  </p>
                  <div className="grid grid-cols-2 gap-2 w-full max-w-sm">
                    {quickCreditSuggestions.slice(0, 4).map((suggestion, index) => (
                      <motion.button
                        key={index}
          ...
          {messages.length > 0 && (
            <div className="px-4 py-2 border-t border-border bg-secondary/10">
              <div className="flex overflow-x-auto space-x-2 pb-2 scrollbar-hide">
                {quickCreditSuggestions.map((suggestion, index) => (
                  <motion.button
                    key={index}
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center">
                        ...
                        <h3 className="text-xl font-display font-bold text-foreground">Asistente Crediticio</h3>
                      </div>
                      <button 
                        onClick={() => setIsInfoModalOpen(false)}
                        className="p-2 rounded-full hover:bg-secondary/50 text-muted-foreground transition-colors"
                    ...
                    <div className="space-y-4">
                      <p className="text-foreground/80">
                        Este asistente utiliza inteligencia artificial para ayudarte a conocer todo sobre creditos hipotecarios
                      </p>
                      ...
                      <div>
                        ...
                        <ul className="space-y-2 text-foreground/70">
                          <li className="flex items-start">
                            ...
                            <span>Documentos para solicitar un credito hipotecario</span>
                          </li>
                          <li className="flex items-start">
                            <Check size={16} className="text-accent mr-2 mt-0.5" />
                            <span>Tipos de creditos hipotecarios</span>
                          </li>
                          <li className="flex items-start">
                            <Check size={16} className="text-accent mr-2 mt-0.5" />
                            <span>Saber si eres elegible para un credito</span>
                          </li>
                        </ul>
                      </div>
                      ...
                      <div className="bg-accent/10 p-4 rounded-lg">
                        <p className="text-foreground/80 text-sm">
                          <span className="font-medium">Nota:</span> Este asistente está en fase beta y sus respuestas son generadas por IA. Siempre verifica la información importante con un broker hipotecario.
                        </p>
                      </div>
                    </div>
                  </motion.div>
  ...
  export default HeliosChatInterface